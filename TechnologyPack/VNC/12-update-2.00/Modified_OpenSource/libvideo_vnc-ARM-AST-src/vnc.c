...
int startCapture()
{
	...
	if (cap_status == CAPTURE_VIDEO_SUCCESS)
	{
		...
		frame_hdr_vnc.params.BytesPerPixel = (((frame_hdr_t *)p_frame_hdr)->FrameHdr_Mode420 != 0 ? BLOCK_SIZE_YUV420 : BLOCK_SIZE_YUV444);
		...
		if ((frame_hdr_vnc.res_x >= VNC_MIN_RESX) && (frame_hdr_vnc.res_y >= VNC_MIN_RESY))
		{
